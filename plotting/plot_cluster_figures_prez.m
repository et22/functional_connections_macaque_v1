...
for k = 1:length(cat_var)
    ...
    h = heatmap(vals,'YDisplayLabels', empty_label, 'XDisplayLabels', empty_label,'MissingDataColor', 'w', 'GridVisible', 'on', 'MissingDataLabel', " ");
    colormap(bone);
    colorbar('off')
...
for k = 2:length(cat_var)
    ...
    cc = [];
    for i = 1:clust_data.num_clusters
        vq= interp2(xo,yo, pair_labels(:,:,i), x,y, 'linear');
        %xs(:,i) = x;
        for aa = 1:size(x*y)
            for bb = 1:size(x*y)
                cc(aa,bb,:) = cmap(i,:);
...
poses = [562   249   242   242;    562    71   327   420; 562    71   327   420];
for k = 1:length(cat_var)
    pre_lab = 'pre_' + cat_var(k);
    post_lab = 'post_' + cat_var(k);
